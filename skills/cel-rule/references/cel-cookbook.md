...
Patterns below are verified against the `cel-rpc-server` evaluation engine. Still, always
round-trip your final expression through `verify_cel_with_tests` — treat this as a starting
...
- **`verify_cel_live_resources` needs List-form expressions.** The single-object shortcut
  does not apply; everything comes back as a List.
